...
public class Reader extends fmi.graph.standard.Reader {
	...
	protected Node readNodeBin() throws IOException {
		return new fmi.graph.chgraph.Node(dis.readInt(), dis.readLong(), dis.readDouble(), dis.readDouble(), dis.readInt(), dis.readInt());
	}
	...
	protected Node readNodeString(String line) throws NoSuchElementException {
		...
		if (split.length == 7) {
			return new fmi.graph.chgraph.Node(Integer.parseInt(split[0]), Long.parseLong(split[1]), Double.parseDouble(split[2]), Double.parseDouble(split[3]),
	...
	protected Edge readEdgeString(String line) throws NoSuchElementException {
		...
		if (split.length == 8) {
			return new fmi.graph.chgraph.Edge(Integer.parseInt(split[0]), Integer.parseInt(split[1]), Integer.parseInt(split[2]), Integer.parseInt(split[3]),
	...
	protected Edge readEdgeBin() throws IOException {
		return new fmi.graph.chgraph.Edge(dis.readInt(), dis.readInt(), dis.readInt(), dis.readInt(), dis.readInt(), dis.readInt(), dis.readInt());
	}
