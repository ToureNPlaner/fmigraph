...
/**
 * Reader for Contraction Hierarchy enabled Graphs
 * beware to cast the returned Edge/Node objects to chgraph.Edge/Node
...
public class Reader extends fmi.graph.standard.Reader {
    ...
    protected Node readNodeBin() throws IOException {
        return new fmi.graph.chgraph.Node(dis.readInt(), dis.readLong(),
                dis.readDouble(), dis.readDouble(), dis.readInt(), dis.readInt());
    }
    ...
    protected Node readNodeString(String line) throws NoSuchElementException {
        ...
        if(split.length==7)
        {
            return new fmi.graph.chgraph.Node(Integer.parseInt(split[0]),	Long.parseLong(split[1]), Double.parseDouble(split[2]),
                    Double.parseDouble(split[3]), Integer.parseInt(split[4]), Integer.parseInt(split[5]),split[6]);
        }
    ...
    protected Edge readEdgeBin() throws IOException {
        return new fmi.graph.chgraph.Edge(dis.readInt(), dis.readInt(), dis.readInt(),
